...
function bindClick(){
    $scope.on("play:change",function(event,index,flag){
        ...
        root.processor.renderAllTime(songList[index].duration)//调用总时间的渲染函数
...
function bindTouch(){
    var $slidePoint = $scope.find(".slider-point");//找到.slider-point
    var offset = $scope.find(".pro-wrapper").offset();
    var left = offset.left;
    var width = offset.width;
    //绑定拖拽事件 开始拖拽 ： 取消进度条渲染
    $slidePoint.on("touchstart",function(){
        root.processor.stop();//拖拽时，音乐暂停
    }).on("touchmove",function(e){//拖拽开始
        //计算拖拽的百分比 更新我们的当前时间和进度条
        var x = e.changedTouches[0].clientX;//获取小圆点移动到的距离
        var percent = (x - left) / width;//拖拽的百分比
        if(percent > 1 || percent < 0){
            percent = 0;
        }
        root.processor.updata(percent)//更新当前进度条
    }).on("touchend",function(e){//拖拽停止
        ...
        var curDuration = songList[controlmanager.index].duration;//歌曲的总时间
